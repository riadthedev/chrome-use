// Global variables to store element data
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  if (message.action === 'getInteractiveElements') {
    try {
      // Store current scroll position and pixel ratio
      ...
      // Run the analysis
      const domTree = analyzeDOM(false); // false means don't highlight, just collect
...
// Main DOM analyzer function
function analyzeDOM(doHighlightElements = false) {
  let highlightIndex = 0; // Reset highlight index

  function highlightElement(element, index, parentIframe = null) {
    // Only perform highlighting if doHighlightElements is true
    if (!doHighlightElements) return index + 1;
  ...
  function buildDomTree(node, parentIframe = null) {
    ...
    if (node.nodeType === Node.ELEMENT_NODE) {
      ...
      // If element meets all criteria, store it in our array
      if (isInteractive && isVisible && isTop) {
        ...
        // Add to our array of interactive elements
        interactiveElements.push(elementInfo);
        
        // Store the index for reference
        nodeData.highlightIndex = elementInfo.index;
        
        // Optionally highlight the element
        if (doHighlightElements) {
          highlightElement(node, elementInfo.index, parentIframe);
        }
      }
    }
